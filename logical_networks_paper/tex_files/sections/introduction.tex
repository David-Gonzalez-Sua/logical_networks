...
In this paper we introduce our ANN representations in the Clingo language. We also introduce the {\tt C-MCPN}: The \emph{Clingo McCulloch-Pitts Network Editor} which can be found at \cite{CMCPN2026}. The C-MCPN is a tool that we developed to assist in the exploration of ASP-based networks. It allows a user to model the network architecture and conduct inference, with underlying representations and inference guided by declarative software written in Clingo. Specifically we restrict our attention to McCulloch-Pitts Networks (MPNs) as the target ANN type \parencite{McCulloch1943}.
